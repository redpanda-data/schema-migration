...
import confluent_kafka.schema_registry.schema_registry_client as client
...
class CustomSchemaRegistryClient(SchemaRegistryClient):
    ...
    def __get_all_versions(self, subject_name):
        ...
        """
        return self._rest_client.get('subjects/{}/versions?deleted=true'.format(client._urlencode(subject_name)))
    ...
    def __get_deleted_version(self, subject_name, version):
        ...
        response = self._rest_client.get('subjects/{}/versions/{}?deleted=true'
                                         .format(client._urlencode(subject_name),
                                                 version))

        schema_type = response.get('schemaType', 'AVRO')
        return client.RegisteredSchema(schema_id=response['id'],
                                       schema=client.Schema(response['schema'],
                                                            schema_type,
